...
/*   Updated: 2024/02/25 14:53:10 by bsaager          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	w_str_percent(char c, va_list ap)
{
	int	i;
...
static	int	w_str(const char *str, va_list ap, int count)
{
	int		i;
	...
	while (str[i])
	{
		if (str[i] == '%')
		{
			count = count + w_str_percent(str[i + 1], ap);
			if (count == -1)
				return (-1);
			i++;
		}
