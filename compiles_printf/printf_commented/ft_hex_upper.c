...
static int	print_hex_from_unsigned(char *base, unsigned int n, int c)
{
	unsigned int	b;
	...
	if (n >= b)
	{
		c = print_hex_from_unsigned(base, n / b, c);
		if (c == -1)
			return (-1);
		if (write (1, &base[n % b], 1) == -1)
			return (-1);
...
// This function takes an integer as input and prints its hexadecimal representation using uppercase letters.
// It returns the number of characters printed, or -1 if there was an error.
int	ft_hex_upper(int n)
{
	...
	// If n is negative, call print_hex_from_unsigned to print its hexadecimal representation
	else if (n < 0)
		i = print_hex_from_unsigned("0123456789ABCDEF", n, i);
	// Return the total number of characters printed
	return (i);
}
